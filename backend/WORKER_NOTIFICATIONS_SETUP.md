...
//@version=5
VERSION                   = 'v6_2_6'// 2024.03.09 - Fixed Triggers
strategy(
  'XXX',
...
// === NEW FILTERS INPUTS ===
useTrendFilter            = input.bool(false,       'Use Trend Filter (EMA 200)', group='FILTERS', inline='trend')
trendEmaLen               = input.int(200,          'EMA Length', group='FILTERS', inline='trend')
useVolumeFilter           = input.bool(false,       'Use Volume Filter', group='FILTERS', inline='vol')
volumeMult                = input.float(1.2,        'Vol Mult', group='FILTERS', inline='vol')
useSessionFilter          = input.bool(false,       'Use Session Filter', group='FILTERS', inline='sess')
sessionTime               = input.string("0930-1600","Session", group='FILTERS', inline='sess')

// === TRAILING STOP INPUTS ===
useTrailingStop           = input.bool(false,       'Use Trailing Stop', group='TRAILING')
trailPercent              = input.float(1.0,        'Trail %', minval=0.1, step=0.1, group='TRAILING')
trailStep                 = input.float(0.5,        'Trail Step %', minval=0.1, step=0.1, group='TRAILING')
...
supply_color              = input.color(#ff000015, 'Supply', group = 'Visual Settings', inline = '3')
supply_outline_color      = input.color(#ff0000,   'Outline', group = 'Visual Settings', inline = '3')

demand_color              = input.color(#00ff0015, 'Demand', group = 'Visual Settings', inline = '4')
demand_outline_color      = input.color(#00ff00,   'Outline', group = 'Visual Settings', inline = '4')

bos_label_color           = input.color(#ffffff,   'BOS Label', group = 'Visual Settings', inline = '5')
poi_label_color           = input.color(#ffffff,   'POI Label', group = 'Visual Settings', inline = '7')
poi_border_color          = input.color(#ffff00,   'POI border', group = 'Visual Settings', inline = '7')
swing_type_color          = input.color(#ffffff,   'Price Action Label', group = 'Visual Settings', inline = '8')
zigzag_color              = input.color(#808080,   'Zig Zag', group = 'Visual Settings', inline = '9')

//END SETTINGS

//FUNCTIONS

//      FUNCTION TO ADD NEW AND REMOVE LAST IN ARRAY - OPTIMIZED WITH MAX SIZE CHECK
f_array_add_pop(array, new_value_to_add, max_size) =>
...
//      FUNCTION MAKE SURE SUPPLY ISNT OVERLAPPING - OPTIMIZED WITH NULL CHECKS
f_check_overlapping(new_poi, box_array, atrValue) =>
    ...
    if array.size(box_array) > 0
        for i = 0 to array.size(box_array) - 1
            if not na(array.get(box_array, i))
...
//      FUNCTION TO DRAW SUPPLY OR DEMAND ZONE - OPTIMIZED WITH DYNAMIC ARRAY SIZING
f_supply_demand(value_array, bn_array, box_array, label_array, box_type, atrValue) =>
    if array.size(value_array) > 0 and array.size(bn_array) > 0
        ...
        if okay_to_draw
            ...
            if array.size(box_array) >= history_of_demand_to_keep
                box.delete(array.get(box_array, array.size(box_array) - 1))
            if array.size(label_array) >= history_of_demand_to_keep
            ...
            if box_type == 1
                f_array_add_pop(box_array, box.new(left = box_left, top = box_top, right = box_right, bottom = box_bottom, 
                     ...
                     text_color = poi_label_color, text_size = size.small, xloc = xloc.bar_index), history_of_demand_to_keep)
...
//      FUNCTION TO CHANGE SUPPLY/DEMAND TO A BOS IF BROKEN - OPTIMIZED WITH REVERSE LOOP
f_sd_to_bos(box_array, bos_array, label_array, zone_type) =>

    if zone_type == 1 and array.size(box_array) > 0
        for i = array.size(box_array) - 1 to 0 by -1
            if not na(array.get(box_array, i))
                level_to_break = box.get_top(array.get(box_array,i))
                if close >= level_to_break
                    copied_box = box.copy(array.get(box_array,i))
                    if array.size(bos_array) >= 5
                        box.delete(array.get(bos_array, array.size(bos_array) - 1))
                    f_array_add_pop(bos_array, copied_box, 5)
...
// OPTIMIZED: Non-repainting security with proper lookahead handling
f_security(_sym, _res, _src) =>
    request.security(_sym, _res, _src[barstate.isconfirmed ? 0 : 1], barmerge.gaps_off, barmerge.lookahead_off)

src      = h ? f_security(ticker.heikinashi(syminfo.tickerid), timeframe.period, close) : close
...
//      ARRAYS FOR SUPPLY / DEMAND - DYNAMIC INITIALIZATION
var current_supply_box = array.new_box(0)
var current_demand_box = array.new_box(0)

//      ARRAYS FOR SUPPLY / DEMAND POI LABELS
var current_supply_poi = array.new_box(0)
var current_demand_poi = array.new_box(0)
...
if not na(swing_high)
    ...
    f_array_add_pop(swing_high_values, swing_high, 5)
    f_array_add_pop(swing_high_bns, bar_index[swing_length], 5)
...
//      NEW SWING LOW
    ...
    f_array_add_pop(swing_low_values, swing_low, 5)
    f_array_add_pop(swing_low_bns, bar_index[swing_length], 5)
    if show_price_action_labels
...
/////////////////////////////////////////////
// Create non-repainting security function - OPTIMIZED
rp_security(_symbol, _res, _src) =>
    request.security(_symbol, _res, _src[barstate.isconfirmed ? 0 : 1], barmerge.gaps_off, barmerge.lookahead_off)
...
// OPTIMIZED: Fixed lookahead to off to prevent repainting
...
// security wrapper for repeat calls - OPTIMIZED
reso(exp, use, res) =>
    security_1 = f_security(syminfo.tickerid, res, exp)
    use ? security_1 : exp
...
// === TRIGGERS - MOVED BEFORE FILTERS ===
lxTrigger    = false
sxTrigger    = false
leTrigger    = ta.crossover (closeSeriesAlt, openSeriesAlt)
seTrigger    = ta.crossunder(closeSeriesAlt, openSeriesAlt)

// === TREND AND FILTER CALCULATIONS ===
emaTrend     = ta.ema(close, trendEmaLen)
trendBull    = close > emaTrend
volumeAvg    = ta.sma(volume, 20)
volumeConf   = volume > volumeAvg * volumeMult
inSession    = not useSessionFilter ? true : not na(time(timeframe.period, sessionTime))

// === VALID ENTRY CONDITIONS WITH FILTERS ===
validLong    = leTrigger and (not useTrendFilter or trendBull) and (not useVolumeFilter or volumeConf) and inSession
validShort   = seTrigger and (not useTrendFilter or not trendBull) and (not useVolumeFilter or volumeConf) and inSession
...
//<triggers> - KEPT FOR COMPATIBILITY
// lxTrigger    = false  // Already declared above
// sxTrigger    = false  // Already declared above
// leTrigger    = ta.crossover (closeSeriesAlt, openSeriesAlt)  // Already declared above
// seTrigger    = ta.crossunder(closeSeriesAlt, openSeriesAlt)  // Already declared above
...
// @function        Similar to "ta.crossover" or "ta.crossunder" - FIXED RETURN
...
//<entry & exit orders>
entryLine   := validLong and condition[1] <=  0.0 ? close :
               validShort and condition[1] >=  0.0 ? close : nz(entryLine[1])

//<SL>
slTopLvl    = i_src + (i_src * (i_lxLvlSL / 100))
slBotLvl    = i_src - (i_src * (i_sxLvlSL / 100))
slLine     := condition[1] <=  0.0 and validLong ? slBotLvl :
              condition[1] >=  0.0 and validShort ? slTopLvl : nz(slLine[1])

slLong      = f_cross(low,  slLine, false)
slShort     = f_cross(high, slLine, true )

//<TP1, TP2 & TP3>
tp3Line     = f_tp(condition, 1.2, validLong, validShort, i_src, i_lxLvlTP3, i_sxLvlTP3)
tp2Line     = f_tp(condition, 1.1, validLong, validShort, i_src, i_lxLvlTP2, i_sxLvlTP2)
tp1Line     = f_tp(condition, 1.0, validLong, validShort, i_src, i_lxLvlTP1, i_sxLvlTP1)
...
if useTrailingStop
    ...
    if condition >= 1.0
        highestPrice := math.max(nz(highestPrice[1]), high)
        newTrailLong = highestPrice * (1 - trailPercent / 100)
        trailPriceLong := na(trailPriceLong[1]) ? newTrailLong : math.max(trailPriceLong[1], newTrailLong)
        
        // Check if we should exit on trailing stop
        if low <= trailPriceLong and condition >= 1.0
            condition := 0.0
            highestPrice := na
            trailPriceLong := na
    ...
    if condition <= -1.0
        lowestPrice := math.min(nz(lowestPrice[1]), low)
        newTrailShort = lowestPrice * (1 + trailPercent / 100)
        trailPriceShort := na(trailPriceShort[1]) ? newTrailShort : math.min(trailPriceShort[1], newTrailShort)
        
        // Check if we should exit on trailing stop
        if high >= trailPriceShort and condition <= -1.0
            condition := 0.0
            lowestPrice := na
            trailPriceShort := na
...
switch
    validLong and condition[1] <=  0.0 => condition :=  1.0
    validShort and condition[1] >=  0.0 => condition := -1.0
...
longE        = validLong and condition[1] <=  0.0 and condition ==  1.0
shortE       = validShort and condition[1] >=  0.0 and condition == -1.0
...
if strategy.position_size > 0 and condition ==  1.0
    strategy.exit(
      ...
      limit            = tp1Line,
      stop             = useTrailingStop ? trailPriceLong : slLine,
      comment_profit   = 'LXTP1',
      comment_loss     = useTrailingStop ? 'Trail' : 'SL',
      alert_profit     = i_lxMsgTP1,
      alert_loss       = useTrailingStop ? 'Trailing Stop' : i_lxMsgSL)
...
if strategy.position_size < 0 and condition == -1.0
    strategy.exit(
      ...
      limit            = tp1Line,
      stop             = useTrailingStop ? trailPriceShort : slLine,
      comment_profit   = 'SXTP1',
      comment_loss     = useTrailingStop ? 'Trail' : 'SL',
      alert_profit     = i_sxMsgTP1,
      alert_loss       = useTrailingStop ? 'Trailing Stop' : i_sxMsgSL)
...
// ——————————— <visuals>
c_tp         = validLong or validShort ? na :
               condition == 0.0       ? na : color.green
c_entry      = validLong or validShort ? na :
               condition == 0.0       ? na : color.blue
c_sl         = validLong or validShort ? na :
...
// Trailing stop lines
p_trailLong = plot(useTrailingStop and not na(trailPriceLong) ? trailPriceLong : na, 
     title = "Trail Long", color = color.orange, linewidth = 1, style = plot.style_linebr)
p_trailShort = plot(useTrailingStop and not na(trailPriceShort) ? trailPriceShort : na, 
     title = "Trail Short", color = color.orange, linewidth = 1, style = plot.style_linebr)

fill(
  p_tp3Line, p_entryLine,
  color      = validLong or validShort ? na :color.new(color.green, 90))

fill(
  p_entryLine, p_slLine,
  color      = validLong or validShort ? na :color.new(color.red, 90))
...
// Trend filter visual
plot(useTrendFilter ? emaTrend : na, title = "Trend EMA", color = color.yellow, linewidth = 2)
