    ...
    return (
        <div className="modal modal-open flex items-center justify-center z-50">
            <div className="modal-box relative max-w-3xl p-8 rounded-xl shadow-2xl border-t-8 border-indigo-500">
                <button
                    className="btn btn-sm btn-circle absolute right-3 top-3 bg-red-500 text-white hover:bg-red-600 print:hidden"
                    onClick={onClose}
                >
                    ✕
                </button>
                ...
                <div className="space-y-6">
                    ...
                    <section className="pb-4">
                        ...
                        {event.devices.map((device) => (
                            <div key={device.id} className="p-4 mt-4 rounded-lg border border-indigo-200">
                                ...
                                <p className={device.restaurant_name ? '':'hidden'}><strong>Ресторан:</strong> {device.restaurant_name}</p>
                                <p className={device.camera_count ? '':'hidden'}><strong>Количество камер:</strong> {device.camera_count}</p>
                                <p><strong>Дата услуги:</strong> {device.event_service_date
                                    ? format(new Date(device.event_service_date), 'dd.MM.yyyy', {locale: ru})
                                    : 'Дата не указана'}</p>
                                <p className={device.comment ? '':'hidden'}><strong>Комментарий:</strong> {device.comment}</p>
                            </div>
                        ))}
                    </section>
                    ...
                    <section className="pb-4">
                        ...
                        <div className="ml-4 space-y-1">
                            <p><strong>Дата создания:</strong> {format(new Date(event.created_at), 'dd.MM.yyyy HH:mm', { locale: ru })}</p>
